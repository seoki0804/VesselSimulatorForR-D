...
class Renderer:
    ...
    def render(self, vessel: BaseVessel, geography: Geography, control: dict, time: float, ais_targets: list[AISTarget]):
        ...
        self._draw_ais_targets(ais_targets) # Draw AIS targets
    ...
    def _draw_ais_targets(self, ais_targets: list[AISTarget]):
        """Draws all AIS targets as triangles."""
        for target in ais_targets:
            ...
            points = [
                (target_size, 0),
                (-target_size / 2, -target_size / 3),
                (-target_size / 2, target_size / 3),
            ]
            
            rot_matrix = np.array([[np.cos(cog_rad), -np.sin(cog_rad)],
            ...
            pygame.draw.polygon(self.screen, (150, 150, 150), screen_points) # Gray target
    ...
    def _draw_vessel(self, vessel: BaseVessel):
        ...
        points = [
            (vessel_len / 2, 0),
            (vessel_len / 4, vessel_width / 2),
            (-vessel_len / 2, vessel_width / 2),
            (-vessel_len / 2, -vessel_width / 2),
            (vessel_len / 4, -vessel_width / 2),
        ]

        rot_matrix = np.array([[np.cos(heading_rad), -np.sin(heading_rad)],
