...
class Renderer:
    ...
    def render(self, vessel: BaseVessel, geography: Geography, control: dict, time: float):
    ...
    def _draw_hud(self, vessel: BaseVessel, control: dict, time: float):
        ...
        if rudder_angle < -0.1:
            rudder_color = (255, 100, 100)
        elif rudder_angle > 0.1:
            rudder_color = (100, 255, 100)
        else:
            rudder_color = (255, 255, 255)

        # Information text (top-left)
        info_texts = [
            f"Time: {time:.1f}s",
        ...
        for i, text in enumerate(control_texts):
            ...
            # Position text on the top right
